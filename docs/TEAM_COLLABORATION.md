...
**3영역 원칙:** `shared`(처음부터 전원 공용 규칙) ↔ `beamsearch/<이니셜>`(개인 자유 탐색) ↔ `team`(승격물 + 공통 데이터). 공정성은 **같은 채점기·같은 금지 가드**를 `shared`로 공유해서 확보하고, 방식·구조·도구는 개인 영역에서 자유. 승격 워크플로우는 [docs/03 §2.4](./03-execution-strategy.md).
